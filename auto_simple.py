...
AUTO_CONFIG = {
    'scroll': {
        'min_speed': 800,  # 最小滚动速度(毫秒)
        'max_speed': 3000,  # 最大滚动速度
        'min_distance': 150,  # 最小滚动距离
        'max_distance': 400,  # 最大滚动距离
        'pause_chance': 0.20,  # 暂停概率
        'min_pause': 3000,  # 最小暂停时间(毫秒)
        'max_pause': 8000,  # 最大暂停时间
    },
    'time': {
        'browse_time': 1800000,  # 浏览时间(毫秒) - 30分钟
        'rest_time': 300000,  # 休息时间 - 5分钟
        'login_wait': 300000,  # 登录等待时间 - 5分钟
    },
    'article': {
        'topic_list_limit': 30,  # 话题列表限制
        'like_chance': 0.4,  # 点赞概率
        'retry_count': 3,  # 重试次数
    },
    'login': {
...
class AutoBrowseController:
    ...
    async def initialize(self):
        ...
        self.browser = await playwright.chromium.launch(
            headless=False  # 使用有头模式，方便远程调试和人工干预
        )
    ...
    async def login(self):
        ...
        if not username or not password:
            self.log("未设置用户名或密码环境变量，等待人工干预", "WARNING")
            # 访问登录页面，等待人工干预
            await self.page.goto('https://linux.do/login')
            await self.wait_for_manual_login(AUTO_CONFIG['time']['login_wait'])
            return True
        ...
        try:
            ...
            # 输入用户名和密码
            await self.page.fill(AUTO_CONFIG['login']['username_field'], username)
            await asyncio.sleep(1.5)
            await self.page.fill(AUTO_CONFIG['login']['password_field'], password)
            await asyncio.sleep(2)
            
            # 点击登录按钮
            await self.page.click(AUTO_CONFIG['login']['login_button'])
            ...
            if 'login' not in current_url:
                ...
                self.log("自动登录失败，等待人工干预", "WARNING")
                return await self.wait_for_manual_login(AUTO_CONFIG['time']['login_wait'])
                
        except Exception as e:
            self.log(f"登录过程中出错: {e}", "ERROR")
            self.log("切换到人工登录模式", "INFO")
            return await self.wait_for_manual_login(AUTO_CONFIG['time']['login_wait'])

    async def wait_for_manual_login(self, wait_time_ms: int) -> bool:
        """等待人工登录"""
        wait_time_seconds = wait_time_ms / 1000
        minutes = int(wait_time_seconds / 60)
        
        self.log(f"等待人工登录，请在{minutes}分钟内完成登录", "WARNING")
        print("\n" + "="*60)
        print(f"\033[1;33m请在{minutes}分钟内手动完成登录操作\033[0m")
        print("="*60 + "\n")
        
        # 每30秒检查一次是否已登录
        check_interval = 30
        checks_total = int(wait_time_seconds / check_interval)
        
        for i in range(checks_total):
            # 显示倒计时
            remaining_minutes = int((wait_time_seconds - i * check_interval) / 60)
            remaining_seconds = int((wait_time_seconds - i * check_interval) % 60)
            
            if i % 2 == 0:  # 每分钟显示一次
                self.log(f"等待人工登录: 还剩 {remaining_minutes}分{remaining_seconds}秒", "INFO")
                
            await asyncio.sleep(check_interval)  # 等待30秒
    ...
    async def get_latest_topics(self):
        ...
        while len(topic_list) < AUTO_CONFIG['article']['topic_list_limit'] and retry_count < AUTO_CONFIG['article']['retry_count']:
            try:
                ...
                self.log(f'获取文章列表失败: {e}', "ERROR")
                retry_count += 1
                await asyncio.sleep(2)

        if len(topic_list) > AUTO_CONFIG['article']['topic_list_limit']:
            topic_list = topic_list[:AUTO_CONFIG['article']['topic_list_limit']]

        # 随机打乱列表顺序，更自然
        random.shuffle(topic_list)
        
        self.topic_list = topic_list
        self.log(f'已获取并随机排序 {len(topic_list)} 篇文章', "INFO")
    ...
    async def like_random_comment(self) -> bool:
        """随机点赞评论"""
        # 根据概率决定是否尝试点赞
        if random.random() > AUTO_CONFIG['article']['like_chance']:
            self.log("根据概率决定此次不点赞", "INFO")
            return False
        ...
        try:
            ...
            if visible_buttons:
                ...
                await asyncio.sleep(random.uniform(1, 2))  # 随机等待时间

                self.log("找到可点赞的评论，准备点赞", "INFO")
                await random_button.click()
                self.likes_count += 1
                self.log(f"点赞成功，当前已点赞数: {self.likes_count}", "INFO")
                await asyncio.sleep(random.uniform(1, 3))  # 点赞后随机等待
                return True
        except Exception as e:
            self.log(f"点赞过程中出错: {e}", "ERROR")
            
        return False

    async def natural_scroll(self, distance):
        """模拟自然的滚动行为"""
        # 将滚动分成多个小步骤，模拟人类滚动
        steps = random.randint(3, 7)  # 随机步数
        step_distance = distance / steps
        
        for i in range(steps):
            actual_step = step_distance + random.uniform(-10, 10)  # 添加随机波动
            await self.page.evaluate(f'window.scrollBy(0, {actual_step})')
            await asyncio.sleep(random.uniform(0.05, 0.15))  # 随机小暂停
    ...
    async def start_scrolling(self):
        ...
        while self.is_scrolling and self.auto_running:
            # 随机滚动速度和距离
            speed = random.randint(AUTO_CONFIG['scroll']['min_speed'], AUTO_CONFIG['scroll']['max_speed'])
            distance = random.randint(AUTO_CONFIG['scroll']['min_distance'], AUTO_CONFIG['scroll']['max_distance'])
            
            # 执行自然滚动
            await self.natural_scroll(distance)
            
            # 随机暂停
            if random.random() < AUTO_CONFIG['scroll']['pause_chance']:
                pause_time = random.randint(
                    AUTO_CONFIG['scroll']['min_pause'],
                    AUTO_CONFIG['scroll']['max_pause']
                )
                self.log(f"随机暂停 {pause_time/1000:.1f} 秒", "INFO")
    ...
    async def accumulate_time(self):
        ...
        total_minutes = AUTO_CONFIG['time']['browse_time'] / 60000
        
        if browse_minutes > 0 and int(browse_minutes) % 5 == 0 and int(browse_minutes * 10) % 50 == 0:  # 每5分钟显示一次
            self.log(f"累计浏览时间: {browse_minutes:.0f}/{total_minutes:.0f} 分钟", "INFO")

        if self.accumulated_time >= AUTO_CONFIG['time']['browse_time']:
    ...
    async def pause_for_rest(self):
        """暂停休息"""
        self.is_scrolling = False
        rest_time_minutes = AUTO_CONFIG['time']['rest_time'] / 60000  # 转换为分钟
        self.log(f"开始休息 {rest_time_minutes:.0f} 分钟", "INFO")
        
        # 显示休息倒计时
        start_time = time.time()
        rest_time_seconds = AUTO_CONFIG['time']['rest_time'] / 1000
...
async def main():
    # 显示启动信息
    print("\n" + "="*60)
    print("\033[1;36m          Linux.do 简易自动浏览工具\033[0m")
    print("="*60)
    print("\033[1;32m功能:\033[0m 自动浏览、随机点赞、人工干预登录")
    print("\033[1;32m作者:\033[0m Auto Simple Browser")
    print("\033[1;32m版本:\033[0m 1.0.0")
    print("="*60 + "\n")
    
    controller = AutoBrowseController()
